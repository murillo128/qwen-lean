...
CONFIG_SCHEMA_VERSION = "qwen35-9b-base-assessment-config-v1"
PREFLIGHT_SCHEMA_VERSION = "qwen35-9b-base-preflight-v1"
EVIDENCE_SCHEMA_VERSION = "qwen35-9b-base-evidence-v1"
...
class Qwen35BaseAssessmentConfig:
    path: Path
    value: dict[str, Any]

    @classmethod
    def load(cls, path: Path) -> Qwen35BaseAssessmentConfig:
    ...
    def validate(self) -> None:
        if self.value.get("schema_version") != CONFIG_SCHEMA_VERSION:
            raise ValueError(
                f"unknown Qwen3.5 9B Base config schema: {self.value.get('schema_version')}"
            )
        model = self.model
        expected_model = {
            "model_id": MODEL_ID,
        ...
        if model != expected_model:
            raise ValueError("model/tokenizer identity differs from issue #43")
        expected_sampling = {
        ...
        if self.sampling != expected_sampling:
            raise ValueError("sampling differs from the strict issue #43 contract")
        if set(self.value["workloads"]) != set(WORKLOADS):
            raise ValueError("assessment workloads differ from issue #43")
        benchmark = self.value["benchmark"]
        if (
    ...
    def _validate_lane(lane: dict[str, Any], *, fallback: bool) -> None:
        ...
        if fallback:
            ...
            quantization = lane.get("quantization_metadata", {})
            expected = {
                "bits": 4,
                "quant_type": "nf4",
...
def validate_model_snapshot(config: Qwen35BaseAssessmentConfig, snapshot: Path) -> Path:
...
def run_preflight(
    ...
    bf16 = _generation_attempt(config, snapshot, config.value["bf16_lane"], [prompt])
    ...
    if bf16["status"] == "passed":
        ...
        fallback = _generation_attempt(
            config, snapshot, config.value["fallback_lane"], [prompt]
        )
...
def _generation_attempt(
    ...
    except BaseException as caught:  # GPU OOM and engine init failures must be retained.
        error = caught
    finally:
        del llm
        gc.collect()
...
def run_assessment(
    ...
    runtime = _local_runtime(config)
    generated, engine_metrics = _generate_candidates(
        config, snapshot, lane, tasks
    )
...
def _validate_preflight(
    ...
    if preflight.get("schema_version") != PREFLIGHT_SCHEMA_VERSION:
        raise ValueError("unknown or missing Qwen3.5 9B Base preflight")
...
def write_compact_evidence(
    ...
    preflight = json.loads(preflight_path.read_text(encoding="utf-8"))
    snapshot = Path(str(preflight.get("model_snapshot")))
    _validate_preflight(config, preflight, snapshot.resolve())
    dev16 = _compact_run(
        config, dev16_dir, expected_workload="minif2f-valid-dev16-v1", expected_tasks=16
    )
    full = _compact_run(
        config, full_dir, expected_workload="minif2f-valid-v1", expected_tasks=244
    )
    ...
    (evidence_dir / "dev16.json").write_text(
        json.dumps(dev16, indent=2, sort_keys=True) + "\n", encoding="utf-8"
    )
...
def _compact_run(
    ...
    *,
    ...
    if metadata.model_id != MODEL_ID or metadata.model_revision != MODEL_REVISION:
        raise ValueError("artifact model identity differs from issue #43")
    if (
        metadata.tokenizer_id != MODEL_ID
        or metadata.tokenizer_revision != MODEL_REVISION
    ):
        raise ValueError("artifact tokenizer identity differs from issue #43")
    ...
    if any(
        ...
        for key in ("chat_template", "prompt_transformation", "proof_extraction", "lean_feedback")
    ):
...
def _render_evidence_readme(payload: dict[str, Any]) -> str:
    ...
    return (
        ...
        "`whole-proof-v1` contract. No chat template, proof extraction, repair, "
        "Lean feedback, or retry was applied.\n\n"
...
def _validate_runtime_versions(
    config: Qwen35BaseAssessmentConfig, vllm_module: Any
) -> None:
    import bitsandbytes
    import torch
    import transformers
...
def _local_runtime(config: Qwen35BaseAssessmentConfig) -> dict[str, Any]:
    import bitsandbytes
    import torch
    import transformers
    import vllm
    ...
    if not torch.cuda.is_available():
        raise RuntimeError("issue #43 requires the project local CUDA GPU")
