...
def test_raw_completion_is_verified_without_historical_prompt_transport() -> None:
    outcome = LeanVerifier(ROOT).verify_raw_completion(
        preamble="import Init",
        model_input="theorem raw_identity (P : Prop) (h : P) : P := by\n  ",
        candidate="exact h",
    )
    assert outcome.category == "verified", outcome.diagnostics
    assert outcome.lean_exit_code == 0
